...
Creates a new guild with a human-readable name. Requires a valid bearer token. Returns HTTP 200 with the created guild record.
...
Creates a channel within the specified guild. Requires a valid bearer token.
...
Appends an optimistic message event to the channel event log. Requires a valid bearer token. The route accepts a JSON payload with `sender` and `content` fields. Both fields must be non-empty. The `sender` must match the authenticated user's identifier; mismatches return HTTP 403.
...
- **Errors**: validation failures return HTTP 400; missing channels return HTTP 404; mismatched sender identities return HTTP 403; missing or invalid access tokens return HTTP 401.
