...
const activeGuildRole = computed(() =>
  resolveGuildRole(activeGuildIdRef.value, sessionGuilds.value),
)
...
const updateRouteQuery = (guildId?: string | null, channelId?: string | null, replace = false) => {
...
const syncFromRoute = async (options: { updateRoute: boolean }) => {
  ...
  try {
    ...
    let targetGuildId =
      typeof route.query.guild === 'string' && route.query.guild.length ? route.query.guild : null
